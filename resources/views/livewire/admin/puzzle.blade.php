<div class="max-w-6xl mx-[40px] py-10 bg-sky-100 rounded-lg shadow-md">
    <h1 class="text-2xl font-bold text-center text-gray-600 mb-6">اطلاعات روز را وارد نمایید</h1>
    ...
    <form wire:submit.prevent="save" class="space-y-6 px-6">
        ...
        <div>
            ...
            <textarea wire:model="question" id="question" rows="5" placeholder="متن..."
        ...
        <div>
            <label for="content" class="block text-sm font-medium text-gray-700">متن</label>
            <textarea wire:model="answer" id="content" rows="5" placeholder="متن..."
                      class="mt-1 w-full p-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-purple-400"></textarea>
            @error('answer')
            <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        ...
        <div class="text-center">
            <button type="submit"
                    class="w-full bg-sky-600 hover:bg-purple-700 text-white font-bold py-3 px-6 rounded-lg transition duration-300 shadow-md">
                ذخیره
            </button>
        </div>
    </form>
    ...
    <br>
    <h1 class="pr-[20px] text-[22px]" >لیست اطلاعات ذخیره شده</h1>
{{--    <p class="pr-[20px] text-[18px]" >تعداد : {{$count}}</p>--}}
    <br>
    <ul>
        @foreach($puzzeuls as $puzzel)
            <li class="pr-[20px]">
                <strong
                    class="absolute left-[5%] "
                ><button wire:click="delete({{ $puzzel->id }})" ><i class="fa fa-trash text-[25px] text-red-600"></i></button></strong>
                <h3>{{$puzzel->title}}</h3><br>
                <p>سوال :{{$puzzel->question}}</p><br>
                <p>جواب : {{$puzzel->answer}}</p><br>
            </li>
            <hr>
        @endforeach
    </ul>
    ...
    <div class="mt-6 flex justify-center ">
        {{ $puzzeuls->links('vendor.pagination.tailwind') }}
    </div>
</div>
