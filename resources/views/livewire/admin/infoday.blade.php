<div class="max-w-6xl mx-[40px] py-10 bg-sky-100 rounded-lg shadow-md">
    ...
    <br>
    <ul>
        @foreach($infos as $infoday)
            <li class="pr-[20px]">
                <strong
                    class="absolute left-[5%] "
                ><button wire:click="delete({{ $infoday->id }})" ><i class="fa fa-trash text-[25px] text-red-600"></i></button></strong>
                <h3>{{$infoday->title}}</h3><br>
                <p>{{$infoday->content}}</p>
            </li>
            <hr>
        @endforeach
    ...
    <div class="mt-6 flex justify-center ">
        {{ $infos->links('vendor.pagination.tailwind') }}
    </div>
</div>
